...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # copy ci_server values from ci_job.ci_server to ci_server
        # copy jobname values from ci_job.jobname to jobname
        for job in orm.UserCiJob.objects.all():
            job.ci_server = job.ci_job.ci_server
            job.jobname = job.ci_job.jobmane


    def backwards(self, orm):
        for job in orm.UserCiJob.objects.all():
            job.ci_job.ci_server = job.ci_server
            job.ci_job.jobname = job.jobmane
